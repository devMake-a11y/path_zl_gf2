...
func NewUDPWriter(addr string, msgMutants ...Mutant) (*UDPWriter, error) {
	var err error
	w := new(UDPWriter)
	w.CompressionLevel = flate.BestSpeed
	w.mutants = msgMutants
...
func (w *UDPWriter) Write(p []byte) (n int, err error) {
	...
	m, err := constructMessage(p, w.hostname, w.Facility)
	if err != nil {
		return 0, err
	}

	for i := 0; i < len(w.mutants); i++ {
		if err = w.mutants[i](p)(m); err != nil {
			return 0, err
		}
	}
